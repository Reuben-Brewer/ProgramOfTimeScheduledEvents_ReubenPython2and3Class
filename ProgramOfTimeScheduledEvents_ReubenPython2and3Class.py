...
class ProgramOfTimeScheduledEvents_ReubenPython2and3Class(Frame): #Subclass the Tkinter Frame
    ...
    def GetMostRecentDataDict(self):
        ...
        if self.EXIT_PROGRAM_FLAG == 0:
            ...
            #deepcopy IS required as MostRecentDataDict contains a queue.
            #HOWEVER, python didn't like this, so I used .copy() on self.MostRecentDataDict and .copy()'ed each EventToCheck dict BEFORE it went into the queue instead.
    ...
    def MainThread(self):
        ...
        while self.EXIT_PROGRAM_FLAG == 0:
            ...
            if self.ProgramCurrentlyRunningFlag == 1 and self.ProgramCurrentlyPausedFlag == 0:
                ...
                #print("EventToCheck: " + str(EventToCheck))
                ...
                if self.CurrentTime_CalculatedFromMainThread - self.ProgramPausedTime_CumulativeSum + TimeSkippedFor_PlayProgramFromThisStartingIndexIntoEventsList >= EventToCheck_TimestampInSeconds:
                    ...
                    #Since we can't deepcopy the entire queue, we'll .copy() each element before we put it in.
                    #Since each EventToCheck dict only contains numbers and strings, .copy() will suffice (deepcopy not needed).
                    self.EventsToFireQueue.put(EventToCheck.copy())
